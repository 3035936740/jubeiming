...
#ifndef JUBEIMING_HTTP_UTIL
#define JUBEIMING_HTTP_UTIL  

namespace jubeiming::utils::HTTPUtil {
	enum status {
	...
	const inline nlohmann::json getJsonResult(status state, std::string_view msg = "", nlohmann::json&& extra = nullptr, std::int32_t status_code = std::numeric_limits<std::int32_t>::min()) {
		std::exchange(extra, extra[0]);
		...
		if (status_code != std::numeric_limits<std::int32_t>::min()) {
	...
	const inline nlohmann::json getJsonResult(status state, nlohmann::json&& extra) {
		return getJsonResult(state, "", std::move(extra));
	}

	const inline nlohmann::json getJsonResult(status state, const std::int32_t status_code, nlohmann::json&& extra) {
		return getJsonResult(state, "", std::move(extra), status_code);
	}

	constexpr inline status getStatus(std::int32_t code) {
		switch (code)
		{
		case Continue:
			return Continue;
	...
	const inline nlohmann::json getSimpleJsonResult(std::int32_t code, std::string_view msg = "", nlohmann::json&& extra = nullptr, std::int32_t status_code = std::numeric_limits<std::int32_t>::min()) {
		return getJsonResult(getStatus(code), msg, std::move(extra), status_code);
	};
}

#endif  // JUBEIMING_HTTP_UTIL